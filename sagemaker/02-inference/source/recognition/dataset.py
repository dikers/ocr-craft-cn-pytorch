    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            if self.opt.rgb:
                ...
                img = Image.new('L', (self.opt.imgW, self.opt.imgH))
        #print('RawDataset, image  {} file {}'.format(img, self.image_path_list[index]))
        return (img, self.image_path_list[index])


class RawCV2Dataset(Dataset):
    """
    文本区域检测生成的数据集，直接进行文本识别， 不进行文件保存。
    """

    def __init__(self, image_obj_list, opt):
        self.opt = opt
